...
import fileSettings from '@/api/file';
...
export const renderFormControl = (field, options = {}) => {
    ...
    switch (type) {
        ...
        // 图片展示字段
        case 'displayImg':
            console.log(fileSettings.baseURL + fieldValue);

            return fieldValue ? <Image className={styles.displayImg} src={fileSettings.baseURL + fieldValue} style={{ ...field.style }} /> : '';
...
export const renderFormItem = (field, options = {}) => {
    ...
    if (dependencies) {
        return (
            <Form.Item
                ...
                noStyle={noStyle} // 原始代码中此场景下有 noStyle
            // 注意：原始代码中此场景下没有 className 和 hidden，保持一致
            >
                {() => field.render(formConnected ? form : null)}
            </Form.Item>
        );
    ...
    // 判断是否为纯展示类型的字段
    const isDisplayType = field.type === 'displayText' || field.type === 'displayImg';
